...
- **出品済みListing一覧画面(§110 step12・第一段階)**: `/listings` で、Publish成功済みのListing(タイトル・SKU・価格・数量・ステータス・出品日・eBay商品ページへのリンク)を一覧表示できる。
  ...
  - **再出品(2026-09-26追加)**: 終了済み(ENDED)のListingに限り、「再出品する」ボタンで同じOfferをeBayへ再度出品できる(`src/app/(app)/listings/relistActions.ts` の `relistListing`)。eBayの`withdrawOffer`はOffer自体を削除するのではなく「非公開」に戻すだけなので、再出品は新しくInventory Item/Offerを作り直す必要はなく、同じOffer IDへ`publishOffer`をもう一度呼ぶだけでよい(eBayが新しいListing IDを発行する)。成功したら新しいeBay Listing IDで`listings`テーブルに別レコードを作成し、古いENDEDレコードは出品終了の履歴としてそのまま残す。
  - これで一覧表示・価格改定・在庫同期・End Item・再出品の一通りの操作が揃った(§110 step12はここでいったん完了とする)。
...
- **出品前AIチェック(指示書§24・2026-09-26追加)**: `/listings/new` のチェックリストの下に「出品前AIチェック」セクションを追加した(APIルートは `src/app/api/ai/prelisting-check/route.ts`)。「出品前チェックを実行」ボタンを押すと、タイトル・ブランド/型番・カテゴリー・Condition・配送/支払い/返品ポリシー・保管場所・価格の入力漏れをAIを使わず機械的にチェックしたうえで(`src/app/api/ai/prelisting-check/route.ts` 内で直接判定・§43/§44: 不要なAI呼び出しをしない)、写真がアップロード済みであれば追加でClaude Visionにより「ブランド・型番が写真の内容と矛盾していないか」「商品説明(英語)が写真の内容と矛盾していないか」「確認できていない事実を断定する表現(禁止表現)が無いか」を判定する(`src/services/ai/precheckListing.ts` の `precheckListingWithAi`)。
  - §25の方針どおり、この結果は警告表示のみでPublish自体をブロックしない(既存のPublishボタンとは独立した任意のチェック)。AIチェックが失敗しても、機械的チェックの結果は表示されるようにしている。
  - §22(商品サイズ・重量)はこのアプリにまだ実装が無いため、「サイズ未入力」「重量未入力」のチェックは対象外(フィールド自体が存在しない)。
